...
(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);throw new Error("Cannot find module '"+o+"'")}var f=n[o]={exports:{}};t[o][0].call(f.exports,function(e){var n=t[o][1][e];return s(n?n:e)},f,f.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){let Asside=require('./modules/asside.js');let GoUp=require('./modules/go-up');let Footer=require('./modules/footer');let ScalePhoto=require('./modules/scale-photo');let Accordion=require('./modules/accordion');let Request=require('./modules/request');let Modal=require('./modules/modal-window');let Form=require('./modules/form');let Comment=require('./modules/comment');let ShowHideComments=require('./modules/show-hide-comments');let ChangeNav=require('./modules/change-nav');document.addEventListener('DOMContentLoaded',()=>{let asside=new Asside().init();let goUp=new GoUp().init();let footer=new Footer().init();let scalePhoto=new ScalePhoto().init();let accordion=new Accordion().init();let getRequest=new Request().getLast();let modal=new Modal();let form=new Form().init();let showHideComments=new ShowHideComments().init();let changeNav=new ChangeNav().init();setTimeout(()=>{let comment=new Comment().init();},1000);});$(window).on('load',function(){let tree=new ClassyLeaves({leaves:50,maxY:5,multiplyOnClick:true,multiply:1,infinite:true,speed:3000,numImages:8,folder:'/assets/leaves/images/leaves/'});let body=document.querySelector('body');let nav=document.querySelector('nav');let mainSection=document.querySelector('.main-section');let foot=document.querySelector('footer');body.style.position='relative';nav.style.opacity='1';mainSection.style.opacity='0.95';foot.style.opacity='1';$('.loader').fadeOut(500);});},{"./modules/accordion":2,"./modules/asside.js":3,"./modules/change-nav":4,"./modules/comment":5,"./modules/footer":6,"./modules/form":7,"./modules/go-up":8,"./modules/modal-window":10,"./modules/request":11,"./modules/scale-photo":12,"./modules/show-hide-comments":13}],2:[function(require,module,exports){class Accordion{constructor(){this.accordion=document.querySelector(".accordion");this.accordionFlush=document.querySelector(".accordion-flush");}
...
module.exports=Asside;},{}],4:[function(require,module,exports){class ChangeNav{constructor(){let Asside=require('./asside');this.asside=new Asside();this.mainBtnNav=document.querySelector('.nav__links .main-btn');this.personsBtnNav=document.querySelector('.nav__links .persons-btn');this.locationsBtnNav=document.querySelector('.nav__links .locations-btn');this.layoutBtnNav=document.querySelector('.nav__links .layout-btn');this.mainBtnAsside=document.querySelector('.asside__links .main-btn');this.personsBtnAsside=document.querySelector('.asside__links .persons-btn');this.locationsBtnAsside=document.querySelector('.asside__links .locations-btn');this.layoutBtnAsside=document.querySelector('.asside__links .layout-btn');this.mainSection=document.querySelector('.main');this.layoutSection=document.querySelector('.layout-section');}
init(){this.mainBtnNav.addEventListener('click',()=>{this.mainSection.style.display='block';this.layoutSection.style.display='none';window.location.href='#';});this.layoutBtnNav.addEventListener('click',()=>{this.layoutSection.style.display='block';this.mainSection.style.display='none';window.location.href='#';})
this.mainBtnAsside.addEventListener('click',()=>{this.mainSection.style.display='block';this.layoutSection.style.display='none';this.asside.hide();window.location.href='#';});this.layoutBtnAsside.addEventListener('click',()=>{this.layoutSection.style.display='block';this.mainSection.style.display='none';this.asside.hide();window.location.href='#';})}}
module.exports=ChangeNav;},{"./asside":3}],5:[function(require,module,exports){class Comment{constructor(){let Request=require('./request');this.request=new Request();this.currentComment=null;this.pswd='17021991';}
...
module.exports=Comment;},{"./request":11}],6:[function(require,module,exports){class Footer{constructor(){this.footer=document.querySelector('.footer-licence');}
init(){let date=new Date();let year=date.toLocaleDateString().slice(6);this.footer.append(`© Made by Gup@lyuk 2021-${year}`);}}
module.exports=Footer;},{}],7:[function(require,module,exports){class Form{constructor(){}
...
Form.link='https://formspree.io/f/xyyvgrww';module.exports=Form;},{}],8:[function(require,module,exports){class GoUp{constructor(){this.getUp=document.querySelector('.get-up');this.section=document.querySelector('.for-go-up');}
init(){window.addEventListener('scroll',()=>{let currentPosition=this.section.getBoundingClientRect().y;if(currentPosition<=109){this.getUp.style.opacity='1';}
if(currentPosition>109){this.getUp.style.opacity='0';}
if(window.innerWidth<=660&&currentPosition<=85){this.getUp.style.opacity='1';}
...
module.exports=ModalWindow;},{"./message":9,"./request":11}],11:[function(require,module,exports){class Request{constructor(){this.root=document.querySelector('.all-comments');this.showHideBtn=document.querySelector('.get-all-comments');}
...
this.root.innerHTML='';this.root.append(...showElements.map(e=>this.createElement(e)));this.root.dataset.hide=dataset;btn.disabled=false;btn.innerHTML=info;}});}else{$.ajax({url:Request.url,method:'get',dataType:'json',success:(data)=>{let showElements=[data[data.length-2],data[data.length-1]]
this.root.innerHTML='';this.root.append(...showElements.map(e=>this.createElement(e)));}});}}
getAll(btn,info,dataset){btn.disabled=true;$.ajax({url:Request.url,method:'get',dataType:'json',success:(data)=>{this.root.innerHTML='';this.root.append(...data.map(e=>this.createElement(e)));btn.disabled=false;btn.innerHTML=info;this.root.dataset.hide=dataset;}});}
post(data){$.ajax({url:Request.url,method:'post',dataType:'json',data:data,success:()=>{setTimeout(()=>{if(this.root.dataset.hide=='true'){this.getLast(this.showHideBtn,'Показать все отзывы','true');}else{this.getAll(this.showHideBtn,'Скрыть отзывы','false');}},1000);}});}
delete(comment){$.ajax({url:`${Request.url}/${comment}`,method:'DELETE',success:()=>{setTimeout(()=>{if(this.root.dataset.hide=='true'){this.getLast(this.showHideBtn,'Показать все отзывы','true');}else{this.getAll(this.showHideBtn,'Скрыть отзывы','false');}},1000);}});}
...
module.exports=ScalePhoto;},{}],13:[function(require,module,exports){class ShowHideComments{constructor(){let Request=require('./request');this.request=new Request();this.showHideBtn=document.querySelector('.get-all-comments');this.allComments=document.querySelector('.all-comments');this.hide=this.allComments.dataset.hide;}
...
module.exports=ShowHideComments;},{"./request":11}]},{},[1])