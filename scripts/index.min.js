...
get(){$.ajax({url:Request.url,method:'get',dataType:'json',success:(data)=>{this.root.innerHTML='';this.root.append(...data.map(e=>createElement(e)))
function createElement(elem){let element=document.createElement('div');element.classList.add('comment');element.dataset.id=`${elem.id}`
...
post(data){$.ajax({url:Request.url,method:'post',dataType:'json',data:data,success:()=>{setTimeout(()=>{this.get();},1000);}});}
delete(comment){$.ajax({url:`${Request.url}/${comment}`,method:'DELETE',success:()=>{setTimeout(()=>{this.get();},1000);}});}}
