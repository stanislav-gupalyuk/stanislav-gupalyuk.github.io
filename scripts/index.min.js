...
init(){this.mainPersons.addEventListener('click',()=>{this.mainPersons.style.boxShadow='inset 0 0 5px rgba(0, 0, 0, 0.5)';this.mainPersons.style.backgroundColor='#ffbc4d';this.mainChoosed.style.display='block';this.mainData.style.display='block';this.mainPersons.style.borderBottom='none';this.secondaryData.style.display='none';this.secondaryChoosed.style.display='none';this.secondaryPersons.style.boxShadow='';this.secondaryPersons.style.borderBottom='1px solid darkseagreen';this.secondaryPersons.style.backgroundColor='#1ea31e';$("#accordion2").accordion({active:'none'});})
this.secondaryPersons.addEventListener('click',()=>{this.secondaryPersons.style.boxShadow='inset 0 0 5px rgba(0, 0, 0, 0.5)';this.secondaryPersons.style.backgroundColor='#ffbc4d';this.secondaryPersons.style.borderBottom='none';this.secondaryChoosed.style.display='block';this.secondaryData.style.display='block';this.mainData.style.display='none';this.mainChoosed.style.display='none';this.mainPersons.style.boxShadow='none';this.mainPersons.style.borderBottom='1px solid darkseagreen';this.mainPersons.style.backgroundColor='#1ea31e';$("#accordion1").accordion({active:'none'});})}}
module.exports=ChoosePersons;},{}],6:[function(require,module,exports){class Comment{constructor(){let Request=require('./request');this.request=new Request();this.currentComment=null;this.pswd='17021991';}
...
module.exports=Comment;},{"./request":12}],7:[function(require,module,exports){class Footer{constructor(){this.footer=document.querySelector('.footer-licence');}
init(){let date=new Date();let year=date.toLocaleDateString().slice(6);this.footer.append(`© Made by Gup@lyuk 2021-${year}`);}}
module.exports=Footer;},{}],8:[function(require,module,exports){class Form{constructor(){}
...
Form.link='https://formspree.io/f/xyyvgrww';module.exports=Form;},{}],9:[function(require,module,exports){class GoUp{constructor(){this.getUp=document.querySelector('.get-up');this.section=document.querySelector('.for-go-up');}
init(){window.addEventListener('scroll',()=>{let currentPosition=this.section.getBoundingClientRect().y;if(currentPosition<=109){this.getUp.style.opacity='0.7';}
if(currentPosition>109){this.getUp.style.opacity='0';}
if(window.innerWidth<=660&&currentPosition<=80){this.getUp.style.opacity='0.7';}
...
module.exports=ModalWindow;},{"./message":10,"./request":12}],12:[function(require,module,exports){class Request{constructor(){this.root=document.querySelector('.all-comments');this.showHideBtn=document.querySelector('.get-all-comments');}
...
this.root.innerHTML='';this.root.append(...showElements.map(e=>this.createElement(e)));this.root.dataset.hide=dataset;btn.disabled=false;btn.innerHTML=info;},error:err=>{alert(`Отзывы не загружены по причине - ${err.statusText}`);btn.disabled=false;}});}else{$.ajax({url:Request.url,method:'get',dataType:'json',success:(data)=>{let showElements=[data[data.length-2],data[data.length-1]]
this.root.innerHTML='';this.root.append(...showElements.map(e=>this.createElement(e)));},error:err=>{alert(`Отзывы не загружены по причине - ${err.statusText}`);}});}}
getAll(btn,info,dataset){btn.disabled=true;$.ajax({url:Request.url,method:'get',dataType:'json',success:(data)=>{this.root.innerHTML='';this.root.append(...data.map(e=>this.createElement(e)));btn.disabled=false;btn.innerHTML=info;this.root.dataset.hide=dataset;},error:err=>{alert(`Отзывы не загружены по причине - ${err.statusText}`);btn.disabled=false;}});}
post(data){$.ajax({url:Request.url,method:'post',dataType:'json',data:data,success:()=>{setTimeout(()=>{if(this.root.dataset.hide=='true'){this.getLast(this.showHideBtn,'Показать все отзывы','true');}else{this.getAll(this.showHideBtn,'Скрыть отзывы','false');}},1000);},error:err=>{alert(`Отзыв не добавлен по причине - ${err.statusText}`);}});}
delete(comment){$.ajax({url:`${Request.url}/${comment}`,method:'DELETE',success:()=>{setTimeout(()=>{if(this.root.dataset.hide=='true'){this.getLast(this.showHideBtn,'Показать все отзывы','true');}else{this.getAll(this.showHideBtn,'Скрыть отзывы','false');}},1000);},error:err=>{alert(`Отзыв не удален по причине - ${err.statusText}`);}});}
...
module.exports=ScalePhoto;},{}],14:[function(require,module,exports){class ShowHideComments{constructor(){let Request=require('./request');this.request=new Request();this.showHideBtn=document.querySelector('.get-all-comments');this.allComments=document.querySelector('.all-comments');this.hide=this.allComments.dataset.hide;}
...
module.exports=ShowHideComments;},{"./request":12}],15:[function(require,module,exports){class ShowTrailer{constructor(){this.root=document.querySelector('.main-section__trailer');this.blur=this.root.querySelector('.video-blur');this.prevVideo=this.root.querySelector('.prev-video');this.trailer=this.root.querySelector('.anim-vid');this.close=this.root.querySelector('.close-vid');this.showTrailerBtn=this.root.querySelector('.see-vid');}
init(){this.showTrailerBtn.addEventListener('click',()=>{if(window.innerWidth<=750){this.prevVideo.style.backgroundColor='snow';this.blur.style.backgroundColor='snow';this.blur.style.opacity='1';this.trailer.style.display='block';this.close.style.display='block';this.showTrailerBtn.style.display='none';}else{this.blur.style.opacity='1';this.trailer.style.display='block';this.close.style.display='block';this.showTrailerBtn.style.display='none';}});this.close.addEventListener('click',()=>{if(window.innerWidth>750){this.blur.style.opacity='0.2';this.trailer.style.display='none';this.close.style.display='none';this.showTrailerBtn.style.display='block';}else{this.prevVideo.style.backgroundColor='';this.blur.style.backgroundColor='black';this.blur.style.opacity='0.2';this.trailer.style.display='none';this.close.style.display='none';this.showTrailerBtn.style.display='block';}})}}
