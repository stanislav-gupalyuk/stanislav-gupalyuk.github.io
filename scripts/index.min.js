...
(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);throw new Error("Cannot find module '"+o+"'")}var f=n[o]={exports:{}};t[o][0].call(f.exports,function(e){var n=t[o][1][e];return s(n?n:e)},f,f.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){let Asside=require('./modules/asside.js');let GoUp=require('./modules/go-up');let Footer=require('./modules/footer');let ScalePhoto=require('./modules/scale-photo');let Accordion=require('./modules/accordion');let Request=require('./modules/request');let Modal=require('./modules/modal-window');let Form=require('./modules/form');let Comment=require('./modules/comment');let ShowHideComments=require('./modules/show-hide-comments');let ChangeNav=require('./modules/change-nav');let ChoosePersons=require('./modules/choose-persons');document.addEventListener('DOMContentLoaded',()=>{let asside=new Asside().init();let goUp=new GoUp().init();let footer=new Footer().init();let scalePhoto=new ScalePhoto().init();let accordion=new Accordion().init();let getRequest=new Request().getLast();let modal=new Modal();let form=new Form().init();let showHideComments=new ShowHideComments().init();let changeNav=new ChangeNav().init();let choosePersons=new ChoosePersons().init();$("#accordion1").accordion({heightStyle:"100%",collapsible:true,active:'none'});$("#accordion2").accordion({heightStyle:"100%",collapsible:true,active:'none'});setTimeout(()=>{let comment=new Comment().init();},1000);});$(window).on('load',function(){let tree=new ClassyLeaves({leaves:50,maxY:5,multiplyOnClick:true,multiply:1,infinite:true,speed:3000,numImages:8,folder:'/assets/leaves/images/leaves/'});let body=document.querySelector('body');let nav=document.querySelector('nav');let mainSection=document.querySelector('.main-section');let foot=document.querySelector('footer');body.style.position='relative';nav.style.opacity='1';mainSection.style.opacity='0.95';foot.style.opacity='1';$('.loader').fadeOut(500);});},{"./modules/accordion":2,"./modules/asside.js":3,"./modules/change-nav":4,"./modules/choose-persons":5,"./modules/comment":6,"./modules/footer":7,"./modules/form":8,"./modules/go-up":9,"./modules/modal-window":11,"./modules/request":12,"./modules/scale-photo":13,"./modules/show-hide-comments":14}],2:[function(require,module,exports){class Accordion{constructor(){this.accordion=document.querySelector(".accordion");this.accordionFlush=document.querySelector(".accordion-flush");}
...
if(window.innerWidth<=660&&currentPosition<=85){this.getUp.style.opacity='0.7';}
if(window.innerWidth<=660&&currentPosition>=85){this.getUp.style.opacity='0';}});this.getUp.addEventListener('click',()=>{window.location.href='#';})}}
...
this.root.innerHTML='';this.root.append(...showElements.map(e=>this.createElement(e)));this.root.dataset.hide=dataset;btn.disabled=false;btn.innerHTML=info;},error:function(err){alert(`Отзывы не загружены по причине - ${err.statusText}`);}});}else{$.ajax({url:Request.url,method:'get',dataType:'json',success:(data)=>{let showElements=[data[data.length-2],data[data.length-1]]
this.root.innerHTML='';this.root.append(...showElements.map(e=>this.createElement(e)));},error:function(err){alert(`Отзывы не загружены по причине - ${err.statusText}`);}});}}
getAll(btn,info,dataset){btn.disabled=true;$.ajax({url:Request.url,method:'get',dataType:'json',success:(data)=>{this.root.innerHTML='';this.root.append(...data.map(e=>this.createElement(e)));btn.disabled=false;btn.innerHTML=info;this.root.dataset.hide=dataset;},error:function(err){alert(`Отзывы не загружены по причине - ${err.statusText}`);}});}
post(data){$.ajax({url:Request.url,method:'post',dataType:'json',data:data,success:()=>{setTimeout(()=>{if(this.root.dataset.hide=='true'){this.getLast(this.showHideBtn,'Показать все отзывы','true');}else{this.getAll(this.showHideBtn,'Скрыть отзывы','false');}},1000);},error:function(err){alert(`Отзыв не добавлен по причине - ${err.statusText}`);}});}
delete(comment){$.ajax({url:`${Request.url}/${comment}`,method:'DELETE',success:()=>{setTimeout(()=>{if(this.root.dataset.hide=='true'){this.getLast(this.showHideBtn,'Показать все отзывы','true');}else{this.getAll(this.showHideBtn,'Скрыть отзывы','false');}},1000);},error:function(err){alert(`Отзыв не удален по причине - ${err.statusText}`);}});}
...
module.exports=ShowHideComments;},{"./request":12}]},{},[1])